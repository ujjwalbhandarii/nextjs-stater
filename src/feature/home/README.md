...
- `types/home.types.ts`: TypeScript interfaces for feature components, nodes, why items, and commands.
- `constants/home.constants.ts`: Separated static data arrays (`WHY_ITEMS`, `ARCHITECTURE_NODES`, `FEATURES`, `COMMANDS_LIST`, `TECH_BADGES`).
- `components/HeroSection.tsx`: Top header with title, tech stack badges, quick action CTAs, and lofi banner image.
- `components/WhySection.tsx`: Showcase grid highlighting core advantages of adopting this template.
...
- Presentational views are decomposed into reusable sub-components (`WhySection`, `ArchitectureTreeGraph`, `ArchitectureNodeDetails`).
